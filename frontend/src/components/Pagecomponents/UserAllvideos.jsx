...
const UserAllvideos = () => {
  ...
  useEffect(()=>{
    const fetchVideos = async ()=>{
        try {
            setLoading(true);
            const response = await axios.get("http://localhost:4000/api/video/uservideos", {withCredentials:true});
            if(response.data){
...
// handle delete video
   const DeleteVideo = async(id)=>{
       try {
        const response = await axios.delete(`http://localhost:4000/api/video/delete/${id}`,{withCredentials:true});
        if(response.data){
